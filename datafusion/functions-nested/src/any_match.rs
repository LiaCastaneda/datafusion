...
impl ArrayAnyMatch {
    pub fn new() -> Self {
        Self {
            ...
            aliases: vec![String::from("list_any_match")],
        }
    }
}
...
impl HigherOrderUDF for ArrayAnyMatch {
    ...
    fn invoke_with_args(&self, args: HigherOrderFunctionArgs) -> Result<ColumnarValue> {
        ...
        // For each row in the input, check if any element in its array satisfies the predicate.
        // We need to map predicate results (flat array over all elements) back to per-row results.
...
mod tests {
    ...
    use crate::any_match::array_any_match_udhof;
        ...
        create_physical_expr(
            &Expr::HigherOrderFunction(HigherOrderFunction::new(
                ...
                vec![
                    ...
                    lambda(
                        ["x"],
                        lambda_var("x", Arc::new(Field::new("x", DataType::Int32, true)))
                            .gt(lit(2i32)),
                    ),
                ],
            )),
