...
pub trait LazyBatchGenerator: Send + Sync + fmt::Debug + fmt::Display {
    ...
    fn generate_next_batch(&mut self, reservation: &MemoryReservation) -> Result<Option<RecordBatch>>;
...
impl ExecutionPlan for LazyMemoryExec {
    ...
    fn execute(
        ...
        // Create memory reservation for tracking arrays generated by this stream
        use datafusion_execution::memory_pool::MemoryConsumer;
        let consumer =
            MemoryConsumer::new(format!("LazyMemoryExec[partition={partition}]"));
...
pub struct LazyMemoryStream {
    ...
    /// Memory reservation for tracking arrays generated by this stream
...
impl Stream for LazyMemoryStream {
    ...
    fn poll_next(
        ...
        let batch = self
            .generator
            .write()
            .generate_next_batch(&self.reservation);
        ...
        println!(
            "[LazyMemoryStream] Before record_poll: reservation.size = {} bytes",
...
mod lazy_memory_tests {
    ...
    impl LazyBatchGenerator for TestGenerator {
        ...
        fn generate_next_batch(
            ...
            let array =
                datafusion_execution::memory_pool::compute::int64_array_from_iter_values(
            ...
            let batch = datafusion_execution::memory_pool::compute::record_batch_try_new(
                Arc::clone(&self.schema),
                vec![Arc::new(array)],
                reservation,
            )?;
    ...
    async fn test_memory_tracking_through_plan() -> Result<()> {
        ...
        // Create TaskContext and get its memory pool
        let task_ctx = Arc::new(TaskContext::default());
        let pool = task_ctx.memory_pool();

        println!("=== Memory Tracking Demo (using TaskContext pool + FilterExec) ===");
        println!("Initial pool state: {} bytes", pool.reserved());

        // Create LazyMemoryExec that will generate arrays with memory tracking
        let generator = TestGenerator {
            counter: 0,
            max_batches: 3,
            batch_size: 10,
        ...
        // Add FilterExec on top (filters rows where a > 5)
        let filter_expr = Arc::new(BinaryExpr::new(
            Arc::new(Column::new("a", 0)),
            Operator::Gt,
            Arc::new(Literal::new(datafusion_common::ScalarValue::Int64(Some(5)))),
        ));

        let filter_exec = Arc::new(FilterExec::try_new(filter_expr, lazy_exec)?);

        // Execute and collect batches
        let mut stream = filter_exec.execute(0, Arc::clone(&task_ctx))?;

        // Pull first batch (FilterExec may coalesce multiple input batches)
        println!("\n--- Pulling batch 1 from FilterExec ---");
        let batch1 = stream.next().await.unwrap()?;
        let after_batch1 = pool.reserved();

        assert!(
            after_batch1 > 0,
            "Pool should track memory after creating batch 1"
        );
        println!("{after_batch1}");

        println!("No second batch (FilterExec coalesced everything into batch 1)");
        drop(batch1);

        let after_drop = pool.reserved();
